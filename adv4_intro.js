(() => {
  ...
  function setNarrative(title, prompt="", ready="") {
    ...
    ui.wrap.dataset.phase = G.intro.phase;
  }
  ...
  function showChapter() {
    ...
    ui.chapterTitle.textContent = "THE LOST NOTES";
    ui.chapterSubtitle.textContent = "Bring them home.";
    ui.chapter.classList.add("visible");
    setTimeout(() => ui.chapter?.classList.remove("visible"), 2600);
  }
  ...
  function enterHome() {
    setPhase("home");
    G.tone(220, .18, .011, "sine");
    setNarrative("WAKE THE LIGHTS.", "This is how the old song begins.", "0 / 4");
  }
  ...
  function wakeHomeLight(light, index) {
    ...
    G.tone(HOME_FREQS[index], .28, .014, index%2 ? "triangle" : "sine");
  ...
  function enterReturn() {
    if (!G.intro.active || !["home","memory"].includes(G.intro.phase)) return;
    setPhase("return");
    G.chord(110, [1,5/4,3/2,15/8]);
    setNarrative("COME HOME.", "", "RETURN TO THE CENTER • SPACE");
  }

  function answerOrigin() {
    if (G.intro.phase !== "return") return;
    if (Math.hypot(p.x-ORIGIN.x, p.y-ORIGIN.y) > 150) {
      G.showMessage("COME CLOSER", 700);
      return;
    }
    ...
    if (G.V16Bond?.beginAnswerRitual) {
      G.V16Bond.beginAnswerRitual(beginBreak);
      return;
    }

    G.chord(110, [1,5/4,3/2,15/8]);
    setNarrative("THE OLD SONG.", "Four notes. Always the same.");
    setTimeout(beginBreak, 1900);
  }
  ...
  function beginBreak() {
    ...
    G.intro.breakAge = 0;
    [1,2,3].forEach(i => {
      G.intro.homeLights[i].broken = true;
      G.intro.homeLights[i].active = false;
    });
    G.V16Bond?.severAnswer?.();
    document.body.classList.add("phaseBreak");
    setTimeout(() => document.body.classList.remove("phaseBreak"), 950);
    G.tone(110, .5, .018, "sine");
    G.tone(155.56, .46, .015, "triangle", .03);
    G.tone(82.41, .75, .012, "sine", .07);
  ...
  function beginRecovery() {
    ...
    if (Array.isArray(s.bonds) && !s.bonds.some(item => item.title === "ORIGIN")) {
      s.bonds.push({
        title:"ORIGIN",
        glyph:"O",
        note:"The first stable pattern P remembers. Three notes were torn from it.",
        effect:"Bring the lost notes home."
      });
    }
    G.addKnown("POSITION", "P has a place to return to. Origin is home.");
    G.addKnown("MOTION", "P can leave home and return. Paths matter because destinations do.");
    hideNarrative();
    if (G.launchOriginHub) {
      G.launchOriginHub();
    } else {
      s.stage = "follow";
  ...
  G.update = dt => {
    ...
    intro.elapsed += dt;
    intro.phaseTime += dt;
    intro.origin.pulse *= Math.pow(.07, dt);
    intro.breakAge += dt;
    s.area = "ORIGIN";
    if (intro.phase === "wake" && intro.phaseTime >= 1.7) {
      enterHome();
      return;
    }
    if (intro.phase === "home") {
